...
研究信息流很吵，Paper Brain 更偏向慢一点但质量更高的阅读流程：

- 每次发现约 10 篇相关论文或项目；
- 优先关注顶会、强实验室和有代码支撑的工作；
- 区分自动导入的元数据和经过验证的精读结果；
- 把图像、笔记、图谱连接、收藏夹和导出汇总放在一个本地工作区；
- 让 Codex 这样的 AI 编程助手帮你跑每日流程，同时不需要把私密凭据写进仓库。

这个干净版仓库只包含示例 seed 节点。它不包含私有论文、PDF、图像裁剪、SQLite 缓存、API key 或个人阅读笔记。
...
离线命令会根据 `config/paper_watch.json` 中的示例 seed 节点生成仪表盘。
...
把 Paper Brain 安装到本地后，你可以让 Codex 这样的 AI 编程助手在仓库里帮你执行每日论文流程。AI 不需要你的 GitHub 密码。如果你启用了外部 API，key 应该只放在 shell 环境变量或本地密钥管理工具中。
...
API key 应放在 shell、`.env` 或系统密钥管理工具中。不要把 key 写进 `config/paper_watch.json`。
...
干净配置默认关闭图像预览抽取。准备好存储生成的图像裁剪后，可以在 `graph.figure_preview` 下启用。
